...
Cut a time series according to diff. frequency trends. Also, takes into account gap info (table from the database)
...
def main ():
    ...
    min_lenght=5  # to cut the series (in terms of num of points)
    ...
    for index_file in range(top):
        ...
        list_cut_times=[]   #OJO!! now i consider cuttings from gaps AND from freq. changes
...
#input file:
       ...
       # i find out if there is a gap in the time series that i need to consider:
                               
        query22= "select * from gaps_by_frequency where ck_id='"+str(ck_id)+"' order by start_day"                
        result22 = db.query(query22)

       

        current_time_serie=[]
        for element in list_series[0]:   # i made a copy of the current time serie  (to cut it according to gaps)
            current_time_serie.append(element)




        print  " index_file:",index_file, " num_gaps:",len(result22),"lenght current time serie:",len(list_series[0])


        if len(result22)>0:  ## if there is a gap in the time series, i have to pre-cut it before i run the algorithm for freq.
            
            list_series=[]   # # i overwrite the previosly un-cut time series, and i substitute it by the fragments
               

       
            last_index=0  # for the case of multiple gaps
            last_end_index=0

            for r22 in result22:   # to cut the time serie
                last_index+=1
                
                segmento=[]

                index_start=r22['index_start_day']
                index_end=r22['index_end_day']
               
            
                if len(result22)==1 :  ######### if only one gap

                    segmento1=[]
                    segmento2=[]                       

                    for index  in range(len(current_time_serie)):   # i save the definitive two segments                       
                        if index+1 <= index_start:     # OJO!!! index+1 to compensate the index diff. between weight change time series and freq time series
                            segmento1.append(current_time_serie[index])   #list of events (time, value,t)                        
                        
                        elif index+1 >= index_end:                           
                             segmento2.append(current_time_serie[index])    
                        else:
                           print "something is wrong", index,current_time_serie[index] ,index_start, index_end
                    ...
                    if len(segmento1)>0: # to avoid isolated points?
                         if segmento1[-1][0] not in list_cut_times:
                             list_cut_times.append(segmento1[-1][0])   # i adda as cutting point the starting mark of the gap
                    if len(segmento2)>0: # to avoid isolated points?
                         if segmento2[0][0] not in list_cut_times:
                             list_cut_times.append(segmento2[0][0])   # i adda as cutting point the ending mark of the gap
                    

                    try:
                        del segmento2[0]     # because now the freq. of the fist point of the second segment is not defined
                    except: pass

                    list_series.append(segmento1)
                    list_series.append(segmento2)





                else:    # if  several gaps


                    if last_index<len(result22):   # if i am NOT analyzing the last segment, according to gaps 

                        segmento1=[]
                        segmento2=[]                       
                    
                        for index  in range(len(current_time_serie)):   # i save the definitive two segments                       
                            if index+1 >= last_end_index:
                                if index+1 <= index_start: # OJO!!! index+1 to compensate the index diff. between weight change time series and freq time series
                                    segmento1.append(current_time_serie[index])   #list of events (time, value,t)                        
                        ...
                        if last_index>1:   # if it is not the first segment
                            try:
                                del segmento1[0]     # because  now the freq. of the fist point of the second segment is not defined
                            except: pass
                        ...
                        try:
                            del segmento2[0]     # now the freq. of the fist point of the second segment is not defined                     
                        except : pass
        ...
        print "num segm. for freq. cutting:",len(list_series),"\n"      
        print index_file,list_cut_times
                ...
                for index in range(num_points):
                    ...
                    if index>=1  and index < num_points-1:  # to cut the serie, at least need one point in each list
                        ...
                        for x1 in range(num_points):
                            ...
                            if (  (len(list_series)>1 and x1>0) or (len(list_series)==1)  ): # if there have been a gap cut, then  i ignore the first point (not correct freq)
                                              
                                if x1 <= index:
                                    list1.append(list_evolution[x1][1]) #only the list of values of freq. (not times!)                           
                                else:
      ...
      #########################  print "\n\nfile:",index_file,"number of cutting points:",len(list_cut_times),"namely:",list_cut_times
        ...
        if len(list_cut_times)!=0:
            ...
            for cut in list_cut_times:
                ...
                if len(list_freq_that_segment)>2:
