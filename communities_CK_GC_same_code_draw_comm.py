...
def main(input_file_name):
  ...
  #  nombre=input_file_name.split("friend_")
   # directorio=nombre[0]
    ...
    file1 = open(input_file_name +'_modularity_analysis_GC','wt') # one summary file for everything
...
# write the output files  
        ...
        net_of_communities(modules,G,H)
...
def net_of_communities(modules,G,H):  # le paso la lista de listas


    com_label=[None]*len(H)#com_label[j]=i: el nodo j pertenece a la comunidad i
    size_com=[None]*len(modules)

   
    for i in range(len(modules)):   # recorro las comunidades (de la GC)
        for j in modules[i]:   #recorro los nodos de una comunidad
            print j
            exit()
            com_label[j]=i
            size_com[i]=len(modules[i])
    ...
    fila=[0]*len(modules)  #inicializo la matriz len(modules) x len(modules)
    strength=[fila]*len(fila) #(sera) lista de listas== matriz de fuerza de los links entre comunidades
