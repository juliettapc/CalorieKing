...
def main ():
    ...
    network_all="./5_points_network_2010/data/full_network_all_users_no_selfloops_merged_small_comm_roles_diff_layers1.5.gml"   
    H = nx.read_gml(network_all)   # just GC, but with Role info
    ...
    for  i in range(8):
        ...
        name_graph_GC="./5_points_network_2010/data/friends_graph_quarter"+i+"_comm_merged_small_comm_roles_diff_layers1.5_attributes.gml"
        name_graph_tot_no_info="./5_points_network_2010/data/friends_graph_quarter"+i+"_comm.gml"



        G = nx.read_gml(name_graph_GC)   # just GC, but with Role info
        ...
        G_comm = nx.read_gml(name_graph_tot_no_info)   # all network, no info
        G_comm = transform_labels_to_nx(G_comm) 
               


        count_nodes_edges(G,G_comm,G_GC,"Q"+i)  # graph_GC all info, graph_tot no info, GC, name to print out
    ...
    for i in range(4):
        ...
        name_graph_GC="./5_points_network_2010/data/friends_graph_sixmonths"+i+"_comm_merged_small_comm_roles_diff_layers1.5_attributes.gml"
        name_graph_tot_no_info="./5_points_network_2010/data/friends_graph_sixmonths"+i+"_comm.gml"
        ...
        count_nodes_edges(G,G_comm,G_GC,"SM"+i)  # graph_GC all info, graph_tot no info, GC, name to print out
...
def count_nodes_edges(G, G_comm, G_GC, name):


    G.add_nodes_from(G_comm.nodes())
    G.add_edges_from(G_comm.edges())
