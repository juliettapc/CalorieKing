...
def main():
    ...
    weigh_ins=2   #filter
    group_study_BMI_under=25   # the users i study have at MOST this bmi   
    phisical_min_bmi= 18   # 32 people have a lower value than this, and are tossed out
    num_groups=1


    bmi_group=1   # to plot   (there are only 2)


    ymin=-12  # ranges for the figure
    ymax=10
    ...
    coeff2_filename="./analysis_time_bins_bmi_groups/coeff_model2_group"+str(bmi_group)+"_normal_users.dat"
    file_coeff2=open(coeff2_filename, 'wt')  

    coeff3_filename="./analysis_time_bins_bmi_groups/coeff_model3_group"+str(bmi_group)+"_normal_users.dat"
  ...
  #(only the first time, then i just read that pickle)
    ...
    name_model1="./analysis_time_bins_bmi_groups/model1_weight_change_vs_t_group"+str(bmi_group)+"_normal_users.txt"
    file1_model=open(name_model1, 'wt')  

    name_model2="./analysis_time_bins_bmi_groups/model2_weight_change_and_wins_vs_t_group"+str(bmi_group)+"_normal_users.txt"
    file2_model=open(name_model2, 'wt')  

    name_model3="./analysis_time_bins_bmi_groups/model3_weight_change_and_wins_and_R6s_vs_t_group"+str(bmi_group)+"_normal_users.txt"
...
#  id ck_id join_date initial_weight most_recent_weight height age weighins initial_bmi final_bmi percentage_weight_change weight_change time_in_system outcome20 outcome50 p_50 act_20 wi_20 p_friend R6_overlap degree friend_avg activity gender paying_info
    ...
    dic_users_2wins_BMI_under25={}   ######## and then i select only the obese & overweight
    for clave in  dic_users_2wins:             
        if dic_users_2wins[clave]['initial_bmi']<group_study_BMI_under  and dic_users_2wins[clave]['initial_bmi'] >= phisical_min_bmi:    
            dic_users_2wins_BMI_under25[clave]=dic_users_2wins[clave]




    ######## i sort obese and overweight users by i_bmi
    list_sorted_dict_users_2wins_BMI_under25=sorted(dic_users_2wins_BMI_under25.items(),key=lambda x: x[1]['initial_bmi'])#,reverse=True)

 #   for item in list_sorted_dict_users_2wins_BMI_under25:   #just to double check
  #     print item[1]['ck_id'],item[1]['initial_bmi']
   

    print "\n\nTotal # users:",len(dict_dict_master),"\nUsers with weigh_ins >=2:", len(dic_users_2wins),"\nUsers with weigh_ins >=2 and BMI<",group_study_BMI_under," and >=",phisical_min_bmi,":", len(dic_users_2wins_BMI_under25)




    ##### ###### i create 10 groups of sorted users according to i_bmi
    size_dict=float(len(dic_users_2wins_BMI_under25))

    ten_percent_users=size_dict/float(num_groups) +1
    first_user_of_group_index=int(ten_percent_users)-1

    first_user_of_group_index=0   
    print "\nGroup first_user_index i_bmi:"
    for i in range(num_groups):        
        print i+1, "    ",first_user_of_group_index, "    ",list_sorted_dict_users_2wins_BMI_under25[first_user_of_group_index][1]['initial_bmi']
        
        if bmi_group == i+1:
            first_i_bmi = list_sorted_dict_users_2wins_BMI_under25[first_user_of_group_index][1]['initial_bmi']

            if bmi_group < num_groups:
                last_i_bmi = list_sorted_dict_users_2wins_BMI_under25[first_user_of_group_index+int(ten_percent_users)][1]['initial_bmi']
            else:
                last_i_bmi = list_sorted_dict_users_2wins_BMI_under25[-1][1]['initial_bmi']
...
#2      2397      23.2946745562
    ...
    dict_users_2wins_BMI_under25_group_selected={}
    for i in range(initial_index,final_index):   
        label=list_sorted_dict_users_2wins_BMI_under25[i][0]
        if len(label)==0:
            raw_input()
        dict_users_2wins_BMI_under25_group_selected[label]=list_sorted_dict_users_2wins_BMI_under25[i][1]
       
      
    num_users_group=len(dict_users_2wins_BMI_under25_group_selected)
    ...
    for clave  in dict_users_2wins_BMI_under25_group_selected:   ########### i create a dictionary of dictionaries for each time bin
        time_system=dict_users_2wins_BMI_under25_group_selected[clave]['time_in_system']

        for item in list_time_bins:
            if time_system >= item:
                dict_dict_users_time_bins[item][clave]=dict_users_2wins_BMI_under25_group_selected[clave]
...
# try to print : mymodel2.t, mymodel2.se etc from:  self.x_varnm, self.b, self.se, self.t, self.p
 ...
 #   ax1.legend( ('Model1', 'Model2', 'Model3'), loc='lower left')   # the colors dont match WHY???
    ...
    figure_title= 'group '+str(bmi_group)+", bmi: "+a+"-"+b+", N="+str(num_users_group)+ " NORMAL users"
    ax1.set_title(figure_title, fontsize=20)
    ax1.set_xlabel('time bins', fontsize=20)
  
   
    figure_name="./analysis_time_bins_bmi_groups/Weight_change_in_bins_models123_bmi_group"+str(bmi_group)+"_normal_users.png"
