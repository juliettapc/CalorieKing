...
def main():
    ...
    filename0="./analysis_time_bins_bmi_groups/master_users_file_for_longitudinal_first6months_constant_w_in_ratio_by_periods.txt"
  ...
  # both networks are only the GC!!!
   ...
   # print "network sizes","   tot:",len(G.nodes()),"   GC:",len(GC_G.nodes()), "  kshell:", len(G_kshell_info_GC.nodes()), "  kshell_GC:", len(GC_G_kshell_info_GC.nodes()), "  engaged:",len(G_kshell_info_engaged.nodes()), "  GC engaged:",len(GC_G_kshell_info_engaged.nodes())   # they are all GC!!!
    ...
    for r1 in result1:
      ...
      #if cont_testing <= 100:    ############### # to test the code out!!
 ...
 ############### weigh_ins records
    ...
    print list_14_periods
    print list_28_periods
  

    for ck_id in dict_master_user_info:   # (for key in dict.)
               
     if len( dict_master_user_info[ck_id]['weigh_in_history']) >0:  # because there are some weird cases were join date < than first w-in in the other table!  (??)
      print "\n\n",ck_id,  dict_master_user_info[ck_id]['weigh_in_history']


      #### i get the average percent w-ins days over 14day window
      dict_day_num_wins_by_period={}
      dict_day_cutting_day={}

      last_day=dict_master_user_info[ck_id]['weigh_in_history'][-1][0]
      last_cutting_day=0
          
    

      for cutting_day in list_14_periods:
          if cutting_day <= last_day+14:
            ...
            #    if day>1:   # i need to exclude the first weigh in for the averages                   
                 ...
                 if day <= list_14_periods[-1]:   # to avoid days after the 6month mark
                    ...
                    if flag_new_period==1:
                        for j in range(len(dict_master_user_info[ck_id]['weigh_in_history'])):
                            new_day=dict_master_user_info[ck_id]['weigh_in_history'][j][0]     
                            if new_day <= cutting_day:     
                                if new_day not in dict_day_num_wins_by_period:
                                    dict_day_num_wins_by_period[new_day]=num_wins
                                    dict_day_cutting_day[new_day]=cutting_day                                    
                                    last_cutting_day=cutting_day                                   
      ...
      for i in range(len(dict_master_user_info[ck_id]['weigh_in_history'])):  # i get the values for the points after the last cut       
          day=dict_master_user_info[ck_id]['weigh_in_history'][i][0]  
      
          if day > last_cutting_day :            
                  dict_day_num_wins_by_period[day]=len(dict_master_user_info[ck_id]['weigh_in_history'])
                  for jj in range(150):   #in case there are several empty periods before the final point
                      new_cutting_day=last_cutting_day+14*jj                  
                      if  new_cutting_day >= day:
                          dict_day_cutting_day[day]= new_cutting_day                            
                          break


      for i in range(len(dict_master_user_info[ck_id]['weigh_in_history'])):  #now i calculate the cte average over periods
                
                day=dict_master_user_info[ck_id]['weigh_in_history'][i][0]  
                num_wins= float(dict_day_num_wins_by_period[day])
                norm= float(dict_day_cutting_day[day])

                avg_14=num_wins/norm *100.       

                pair=[]
                pair.append(day)
                pair.append(avg_14)
                dict_master_user_info[ck_id]['percent_wins_14'].append(pair)

              #  print day, dict_day_num_wins_by_period[day], dict_day_cutting_day[day], avg_14
    ...
    #  print"\n"
      #### i get the average percent w-ins days over 28day window
      dict_day_num_wins_by_period={}
      dict_day_cutting_day={}
      ...
      for cutting_day in list_28_periods:
          if cutting_day <= last_day+28:
            ...
            #    if day>1:   # i need to exclude the first weigh in for the averages                   
                 ...
                 if day <= list_28_periods[-1]:   # to avoid days after the 6month mark
      ...
      for i in range(len(dict_master_user_info[ck_id]['weigh_in_history'])):  # i get the values for the points after the last cut       
          ...
          if day > last_cutting_day :            
                  dict_day_num_wins_by_period[day]=len(dict_master_user_info[ck_id]['weigh_in_history'])
                  for jj in range(150):   #in case there are several empty periods before the final point
                      new_cutting_day=last_cutting_day+28*jj                  
                      if  new_cutting_day >= day:
                          dict_day_cutting_day[day]= new_cutting_day                            
                          break
      ...
      for i in range(len(dict_master_user_info[ck_id]['weigh_in_history'])):  #now i calculate the cte average over periods
                ...
                avg_28=num_wins/norm *100.       

                pair=[]
                pair.append(day)
                pair.append(avg_28)
                dict_master_user_info[ck_id]['percent_wins_28'].append(pair)

              #  print day, dict_day_num_wins_by_period[day], dict_day_cutting_day[day], avg_28
