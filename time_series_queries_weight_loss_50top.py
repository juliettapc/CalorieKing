...
def main ():
    ...
    for r1 in result1:   #loop over users to get their number_of_weigh-ins
       ...
       # control+=1
        #if control >=test:   #QUITAR ESTO EN LA VERSION FINAL!!!!!
...
# one file per each user's weight time serie:
            ...
            for row in result2: 
                ...
                if (float(row['weight'])>10.0): # to eliminate some error data   
                    ...
                    print >> file, contador2,row['weight'],(float(row['weight'])-initial_weight)*100.0/initial_weight,float(row['weight'])*703.0/(float(r1['height'])*float(r1['height'])),row['on_day']-first_day ,row['on_day']
