...
#OJO! QUITAR LA SEGUNDA CONDICION EN LA VERSION DEFINITIVA!!
        ...
        if thread not in threads and len(threads) <= 1000:  # if this thread hasnt been counted yet
...
# thread_list_of_dicc  is list of diccionaries: [{'thread_id': , 'node_label': , 'members': [],'size': , 'num_posts': }, {}, ...]
    ...
    for g in thread_list_of_dicc:  #loop over nodes (=threads)
        print g['node_label']
        for j in thread_list_of_dicc: #loop over nodes (=threads)
           
            if j['node_label'] > g['node_label']: # i compare different threads, looking for common posters
               # g, j  are two diccionaries
              
                node1=g['node_label']
                node2=j['node_label']
                #print node1, node2
             
                #print g['members'],j['members']
                #raw_input()
    ...
    nx.write_gml(G,"Forums_graph.gml")
