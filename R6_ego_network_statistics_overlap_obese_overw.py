...
def main(graph_name):
    ...
    cont=0
    for n in G.nodes():
        if float(G.node[n]['initial_bmi'])>25.0:
            cont+=1



    print "final size of the GC:",len(G.nodes()),"obese & overweight:",cont
    ...
    name0=dir+name+"_overlap_R6s_averages_"+str(time_in_system)+"days_exclude_R6s_obese.dat"
    file0=open(name0, 'wt')    
    file0.close()
    ...
    contador=0
    name12=dir+name+"_slopes_for_the_fits_average_weight_change_obese.dat"           
    file=open(name12, 'wt')
    file.close()
...
####for the Isolated Clusters:
 ...
 ###########################  
    ...
    name00=dir+name+"R6s_and_top_tens_averages_"+str(time_in_system)+"days_exclude_R6s_obese.dat"
...
# count number of 2, 3, 4, 5, 6  and 7cliques:
 ...
 #   print " 2: ",num_2cliques, "     3: ",num_3cliques, "   4: ",num_4cliques, "     5: ",num_5cliques, "   6: ",num_6cliques, "   7: ",num_7cliques, "   8: ",num_8cliques, "   9: ",num_9cliques, "   max_clique_size:",max_clique, "   num_tot_cliques:", num_tot_clique
    ...
    name33=dir+name+"_percent_weight_change_vs_largest_clique_size_obese.dat" 
    ...
    for clique_size in range(1,max_clique):
        ...
        for n in G.nodes():
            ...
            if (G.node[n]['max_clique_size']==clique_size)  and (float(G.node[n]['initial_bmi'])>25.0) :
    ...
    name11=dir+name+"_fits_clique_size_obese.dat"
...
#######################################
    ...
    name11=dir+name+"_dose_eff_indepently_only_one_R6_"+str(time_in_system)+"days_exclude_R6s_obese.dat" 
        ...
        for node in G.nodes():
            ...
            if int(G.node[node]["R6_overlap"])==r:
                ...
                if G.node[node]["role"]== "R6":  # i exclude the R6s                    
                    ...
                    if (int(G.node[node]['time_in_system']) > time_in_system) and (float(G.node[node]['initial_bmi'])>25.0):                                               
...
#    x_positions_fit.append(numpy.mean(averages_larger5_x)/norm)
 #   y_positions_fit.append(numpy.mean(averages_larger5_y)/norm)
    ...
    name11=dir+name+"_fits_dose_eff_R6_obese.dat"
...
#print out
        name2=dir+name+"_ego_R6s_average_weight_change_"+str(time_in_system)+"days_obese.dat"
        file2=open(name2, 'at')
        print >> file2,cont,G.node[node]['role'],G.node[node]['label'],len(G.neighbors(node)),average_weight_change,deviation_weight
        file2.close()


        name22=dir+name+"_ego_R6s_average_percentage_weight_change_"+str(time_in_system)+"days_obese.dat"
        ...
        name3=dir+name+"_ego_R6s_average_activity_"+str(time_in_system)+"days_obese.dat"
        file3=open(name3, 'at')
...
############## percentage weight change vs k
    ...
    file = open(dir+name+"_percentage_weight_change_vs_k_obese.dat",'wt')
    max_k=max_k+1
    for k in range(1,max_k):   
        ...
        for n in G.nodes():
            if (len(G.neighbors(n))==k)  and int((G.node[n]['initial_bmi'])>25.0):
    ...
    name11=dir+name+"_fits_degree_obese.dat"
