...
class ols:
    ...
    def summary_to_file(self, filename):
        """
        Printing model output to file
        """

        file_object= open(filename, 'at')  
        ...
        # printing output to screen
        print >> file_object, '\n=============================================================================='
        print >> file_object, "Dependent Variable: " + self.y_varnm
        print >> file_object, "Method: Least Squares"
        print >> file_object, "Date: ", time.strftime("%a, %d %b %Y",t)
        print >> file_object, "Time: ", time.strftime("%H:%M:%S",t)
        print >> file_object, '# obs:               %5.0f' % self.nobs
        print >> file_object, '# variables:     %5.0f' % self.ncoef 
        print >> file_object, '=============================================================================='
        print >> file_object, 'variable     coefficient     std. Error      t-statistic     prob.'
        print >> file_object, '=============================================================================='
        for i in range(len(self.x_varnm)):
            print >> file_object, '''% -5s          % -5.6f     % -5.6f     % -5.6f     % -5.6f''' % tuple([self.x_varnm[i],self.b[i],self.se[i],self.t[i],self.p[i]]) 
        print >> file_object, '=============================================================================='
        print >> file_object, 'Models stats                         Residual stats'
        print >> file_object, '=============================================================================='
        print >> file_object, 'R-squared            % -5.6f         Durbin-Watson stat  % -5.6f' % tuple([self.R2, self.dw()])
        print >> file_object, 'Adjusted R-squared   % -5.6f         Omnibus stat        % -5.6f' % tuple([self.R2adj, omni])
        print >> file_object, 'F-statistic          % -5.6f         Prob(Omnibus stat)  % -5.6f' % tuple([self.F, omnipv])
        print >> file_object, 'Prob (F-statistic)   % -5.6f			JB stat             % -5.6f' % tuple([self.Fpv, JB])
        print >> file_object, 'Log likelihood       % -5.6f			Prob(JB)            % -5.6f' % tuple([ll, JBpv])
        print >> file_object, 'AIC criterion        % -5.6f         Skew                % -5.6f' % tuple([aic, skew])
        print >> file_object, 'BIC criterion        % -5.6f         Kurtosis            % -5.6f' % tuple([bic, kurtosis])
        print >> file_object, '=============================================================================='

        file_object.close()

        return self.x_varnm, self.b, self.se, self.t, self.p  # all the  names and values of the coeff., sd, tstat and pvalue
