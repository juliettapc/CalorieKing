...
#  ck_id label_id weight_change_4months weight_change_6months pwc_4months pwc_6months num_wins_4months num_wins_6months num_activity_4months num_activity_6months initial_BMI gender age height paying_info join_date join_time time_in_system p_120 p_180 p_friends p_120_friends p_180_friends outcome20 act20 wi_20 num_friends avg_w_change_friends betweenness num_R6_friends k_shell_index max_clique_size role 
    ...
    list_variables_networked_people=[]
    cont_lines=0
     ...
     if cont_lines >0:
        ...
        try:
            if int(dict_master_user_info[ck_id]['num_friends']) >0:
                ...
                lista=[dict_master_user_info[ck_id]['num_friends'],dict_master_user_info[ck_id]['betweenness'],dict_master_user_info[ck_id]['num_R6_friends'],dict_master_user_info[ck_id]['k_shell_index'],dict_master_user_info[ck_id]['max_clique_size'],dict_master_user_info[ck_id]['role'] ]

                list_variables_networked_people.append(lista)
    ...
    csv_file_rand="/home/staff/julia/at_Northwestern/calorieking/calorie_king_hg/data2009_2010/analysis_time_bins_bmi_groups/master_files_randomized_weight_network/master_users_file_weight_change_first6months_p180_p120_within_randomized_weight_network_people_keeping_ibmi_n_wins.txt"
    file1 = open(csv_file_rand,'wt')
    print >> file1, "ck_id label_id weight_change_4months weight_change_6months pwc_4months pwc_6months num_wins_4months num_wins_6months num_activity_4months num_activity_6months initial_BMI gender age height paying_info join_date join_time time_in_system p_120 p_180 p_friends p_120_friends p_180_friends outcome20 act20 wi_20 avg_w_change_friends",
    

    for i in range(Niter):
        print >> file1, "num_friends_rand"+str(i), "betweenness_rand"+str(i), "num_R6_friends_rand"+str(i), "k_shell_index_rand"+str(i), "max_clique_size_rand"+str(i), "role_rand"+str(i),
    ...
    list_lists_randomized_values=[]
    for i in range(Niter):
        new_list_randomized_variables=random.sample(list_variables_networked_people,len(list_variables_networked_people) )  # (sample without replacement)
        list_lists_randomized_values.append(new_list_randomized_variables)
    ...
    for index in range(len(list_ck_id)):  # loop over all users
        ...
        if ck_id in list_networked_users:
            ...
            for i in range(Niter):
                ...
                for index_item in range(len(lista)):   # lista contains all network attributes of a particular user
                    print >> file1,list_lists_randomized_values[i][cont_networked_users][index_item],    
            ...
            for i in range(Niter):
                print  index, i,cont_networked_users
                print >> file1,dict_master_user_info[ck_id]['num_friends'],dict_master_user_info[ck_id]['betweenness'],dict_master_user_info[ck_id]['num_R6_friends'],dict_master_user_info[ck_id]['k_shell_index'],dict_master_user_info[ck_id]['max_clique_size'],dict_master_user_info[ck_id]['role'],  # for non-network people, there is no randomization
