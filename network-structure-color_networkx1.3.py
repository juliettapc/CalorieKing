...
#    print "generating plots using " + str(max_hubs) + " hubs"
    ...
    G = nx.connected_component_subgraphs(G)[0]   # take just the GC or all
    ...
    H = nx.Graph() #### For undirected graph use H = nx.Graph()
    H.add_edges_from(G.edges())
#    print G.edges()
    activity = []
#    nodepos = []
    pos=nx.graphviz_layout(H,prog='neato',args='') #'twopi':radial layout,'fdp': tb spring, 'neato':spring
...
#        nodepos.append(int(nodelabel))
    ...
    for n in topn:
       ...
       size_node = 365*G.node[n]['n_weight_change']
       if size_node > 0 :
...
#    formatter=ticker.LogFormatterMathtext()
    ...
    pyplot.title('Normalized weight-loss (circle) and Normalized weight-gain (square)')
    pyplot.axis('off')
    pyplot.show()
