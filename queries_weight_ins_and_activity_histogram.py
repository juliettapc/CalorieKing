...
def main ():
    ...
    histogram_activity=[0]*100000
    cum_histogram_activity=[0]*100000
    ...
    for r1 in result1:   #loop over users to get their number_of_weigh-ins
       ...
       # result2 is a list of dict.: [{'id': 163978L, 'activity_flag': u'WI', 'weight': 144.0, 'on_day': datetime.datetime(2009, 11, 27, 0, 0), 'ck_id': u'bd84dbe2-dd6e-4125-b006-239442df2ff6'}, {'id': 163979L, 'activity_flag': u'WI', 'weight': 143.09999999999999, 'on_day': datetime.datetime(2009, 12, 15, 0, 0), 'ck_id': u'bd84dbe2-dd6e-4125-b006-239442df2ff6'}, ...,]
            ...
            query3=" select * from activity_combined where (activity_flag not like 'WI' and ck_id like '"+str(ck_id)+"')  order by activity_date  asc"
            ...
            r1['num_act']=len(result3)

            print "# wi:",r1['num_wi'],"# act:",r1['num_act']
            ...
            histogram_activity[r1['num_act']]+=1    #histograma tot num of events




            
            if r1['num_act']>0:  #only if the user has an activity record
                event1=result3[0]['activity_date']   #histogram interevent times between activities
                for i in range(len(result3)):
                    if i>0:  # skip the first event
                        event2=result3[i]['activity_date']
                        interevent_time_days=int((event2-event1).days)   #OJO!!! podria ser freq. de horas o minutos!!
                        total_histogr_interevent_act[interevent_time_days]+=1
                        cont_interevent +=1
    ...
    for i in range(len(histogram_activity)):
        for j in range(len(histogram_activity)):
            if j>=i:               
                cum_histogram_activity[i]+=histogram_activity[j]




    name="temporal_series/most_weigh_ins/histogram_num_activity.dat" 
    file = open(name,'wt')
    for i in range(len(histogram_activity)):
        if histogram_activity[i]!=0:
            print >> file, i, histogram_activity[i]/float(contador),cum_histogram_activity[i]/float(contador)
    file.close()
