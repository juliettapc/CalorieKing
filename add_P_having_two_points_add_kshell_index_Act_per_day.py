...
def main():
    ...
    print >> file,"id ck_id join_date join_time initial_weight most_recent_weight height age weighins initial_bmi final_bmi percentage_weight_change weight_change time_in_system outcome20 outcome50 p_50 act_20 wi_20 p_friend R6_overlap degree friend_avg activity P_2points kshell  act_per_day"
            ...
            if node_exists==1:
                ...
                if str(row[14])=="NA":                
                    for i in range(len(row)):
                        print >> file,row[i],
                    print >> file,0,H.node[node]["kshell_index"], float(H.node[node]["activity"])/float(H.node[node]["time_in_system"])                  
                    
                else:               
                    for i in range(len(row)):
                        print >> file,row[i],
                    print >> file,1,H.node[node]["kshell_index"], float(H.node[node]["activity"])/float(H.node[node]["time_in_system"])   
                ...
                if str(row[14])=="NA":                
                    for i in range(len(row)):
                        print >> file,row[i],
                    print >> file,0,0,float(row[22])/float(row[12])                  
                        
                else:               
                    for i in range(len(row)):
                        print >> file,row[i],
                    print >> file,1,0,float(row[22])/float(row[12])    
