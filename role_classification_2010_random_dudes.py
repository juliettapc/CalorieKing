...
#name=5_points_network_2010/data/new_networks/6months/friends_graph_sixmonths1_comm.gml
    ...
    #dir=name.split('meth')[0]
    
    
    
    info_name=name.split('.gml')[0]   
    #info_name=info_name.split('3method/')[1]
...
# asign a role to each node:
    for i in list_nodes_GC:
        ...
        #raw_input()    
        if G.node[i]['zi']>= 10.0: #the special R6s
            if G.node[i]['Pi']< 0.3:
    ...
    for i in G.nodes():
        if str(G.node[i]['label'])=="40155":
            node=i
            break


    list_friends_only_with_special_guy=[]
    for n in G.neighbors(node):
        if len(G.neighbors(n))==1:
            list_friends_only_with_special_guy.append(n)

    print len(list_friends_only_with_special_guy),"guys only connected to 40155"

    raw_input()



#asigng RANDOM roles to 41 dudes:
    cont_dudes=0
    list_dudes=[]
    while cont_dudes < num_dudes :
        i=int(random.random()*len(list_nodes_GC))
        node=list_nodes_GC[i]
        if (node not in list_dudes) and (node not in list_friends_only_with_special_guy): # exclude the guys only connected to the special one
            list_dudes.append(node)
            G.node[node]['role']="random_role"
            print node, "choosen as random dude"
            cont_dudes=cont_dudes+1
...
# write zscore-Participation to a file
    file1 = open(dir+info_name+"_zscore_vs_ParticipationCoef.dat",'wt')
    for i in list_nodes_GC:
...
# write the list of lists (communities) sorted by role to a file
    ...
    file2 = open(dir+"list_of_communities_"+info_name+"_by_role",'wt')
...
# write zscore-Participation with info about communities to a file
    file3 = open(dir+info_name+"_zscore_vs_ParticipCoef_with_comm_info.dat",'wt')
...
#create a new gml file with the added atributes:
    ...
    nx.write_gml(H,name+"_roles_random.gml")
