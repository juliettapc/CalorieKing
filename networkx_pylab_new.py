...
def draw_networkx_edges(G, pos,
    ...
    if G.is_directed():
        ...
        for src,dst in edge_pos:
            ...
            dy=y2-y1 # y offset
            dr = sqrt(dx**2+dy**2)
            ddr=0.5*sqrt(node_size)/sqrt(200.)
            ddx = ddr*dx/dr 
            ddy = ddr*dy/dr
...
#                ya=p*d*numpy.sin(theta)+y1
            ...
            arrow_patches.append(FancyArrow(x1,y1,dx-ddx,dy-ddy,edgecolor = 'k',head_width=15,length_includes_head=True))

        arrow_verts = [a.get_verts() for a in arrow_patches]

        # arrows replaces edges
        arrow_collection = LineCollection(arrow_verts,
                                colors   = arrow_colors,
                                linewidths   = lw,
                                antialiaseds = (1,),
                                transOffset  = ax.transData,             
