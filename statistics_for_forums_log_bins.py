...
Calculates the distribution of: number of posters, number of posts, time intervals between posts, total time elapsed from first to last post in each thread.
...
def main ():
    ...
    cum_distr_posts=[0.0]*num_threads
    ...
    for r1 in result1:  # loop over different threads
        ...
        if count <= 100000:   # DELETE THIS CONDITION IN THE FINAL VERSION!!!!!!!!
            ...
            if num_posts >1:  # exclude the threads with only one post
                ...
                for i in (1,num_posters+1):    # cumulative distribution <=i
                    cum_distr_posters[i]=cum_distr_posters[i]+1
                ...
                for i in (1,num_posts+1):    # cumulative distribution <=
                    cum_distr_posts[i]=cum_distr_posts[i]+1
...
#logarithmic bins (in seconds):
    ...
    file4 = open("Final_initial_time_boxes_CK_forums_log_5.dat",'wt')
...
#logarithmic bins (in seconds):
    ...
    for i in list_delta_times_posts:       
     ...
     # print out the distribution of final-initial post times:
    file5 = open("Delta_times_between_posts_boxes_CK_forums_log_5.dat",'wt')
    for i in range(len(number_events_box)):
