...
#input_file_name=friends_undirected_all_quarter0



    nombre=input_file_name.split("friend")
    directorio=nombre[0]
    ...
    graph_name=directorio+"friends_graph_quarter0_comm_roles.gml"
    G_roles = nx.read_gml(graph_name)
    ...
    for name in name_list: # loop to go over files (== over networks)
        ...
        for p in partition_lines:
            ...
            if len(this_module) > 1:
                ...
                this_module_role_restricted=[i for i in this_module if (G_roles.node[i]['role']==role)]


                print this_module,this_module_role_restricted 

                raw_input()

                modules_role_restricted.append(this_module_role_restricted)
