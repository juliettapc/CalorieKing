...
def get_activity_count(uid, db, year_restrict):
    #get all uids
    result = db.query("SELECT id, ck_id FROM users")
    candidates = []
        ...
        #get the number of weigh-in points
    if restrict:
        act_count = db.query("SELECT COUNT(*) FROM activity_combined where ck_id='"+str(uid)+"' AND on_day >= '2009-01-01'")
    else:
        act_count = db.query("SELECT COUNT(*) FROM activity_combined where ck_id='"+str(uid)+"' AND\
        activity_date >= '2009-01-01' AND activity_flag <> 'WI'")

    return int(act_count[0]['COUNT(*)']) 
 

def get_activity_history(uid, db, inc_weigh_ins, year_restrict=None):
    
    if inc_weigh_ins:
        q = "SELECT act.activity_date, act.activity_flag FROM activity_combined as act WHERE ck_id ='"+uid+"' AND activity_date >= '2009-01-01'"   
...
def main(num_points, year_restrict=None, inc_weigh_ins = True, server="tarraco.chem-eng.northwestern.edu", user="calorieking", passwd="n1ckuDB!"):
    ...
    #else:
    f = open("./"+str(num_points)+"_points_network_2010/data/activity_in_candidates_with_"\
                +str(num_points)+"_total_points.dat", "w")

    print >> f, '"id","ck_id","activity"'
    
    ids = [[x.strip().split(",")[0],x.strip().split(",")] for x in
    open("./1_points_network_2010/data/all_1.csv").readlines()[1:]]


    for uid in ids:
        t = get_activity_count(uid[1], db, year_restrict) 
        print >> f, ",".join(uid),",", t[0]
    f.close()
    ...
    for uid in uids:
        ...
        activity_count = get_activity_count(uid[1], db, year_restrict)
    ...
    g = open(dirname+"/"+str(num_points)+"_point_users_attribute_data_"+str(num_points)+"_points_2.dat", "w")
    
    print "dirname", dirname
    
    print>>g, '"id", "ck_id", "weigh_ins", "initial_weight",\
    "weight_change","days","n_weight_change", "percentage_weight_change", "time_in_system",\
    "height", "age", "initial_bmi", "final_bmi", "change_in_BMI","last_day_of_act",\
    "days_since_last_act", "activity"'
