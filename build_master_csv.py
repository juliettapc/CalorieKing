...
def compute_metric(metric, row, metric_type):
    ...
    if metric == "initial_bmi":
        ...
        result_metric = compute_outcome(row)
    elif metric =="act20":
        result_metric = compute_activity_20(row)
    elif metric =="activity":
        result_metric = compute_activity_20(row)
    elif metric == "wi20":
        result_metric = compute_weighins_20(row)
    elif metric == "outcome_percent":
        result_metric = compute_outcome_percent(row)
...
def compute_weighins(row):
    ...
    count_query ="SELECT COUNT(*) FROM weigh_in_history WHERE ck_id = '"+ck_id+"' AND on_day >='"+str(join_date)+"'"
    count = int(db.query(count_query)[0]['COUNT(*)'])
    return count


def compute_activity_20(row):
    db = DBConnection()
    ck_id = row['ck_id']
    join_date = str(row['join_date'].date())
    cut_off = str(datetime(*strptime(join_date, "%Y-%m-%d")[0:6]) + relativedelta(days = 20))
    count_query ="SELECT COUNT(*) FROM activity_combined WHERE activity_flag <> 'WI' AND ck_id ='"+ck_id+"' AND activity_date >='"+str(join_date)\
    +"' AND activity_date < '"+cut_off+"'"
    count = int(db.query(count_query)[0]['COUNT(*)'])
    return count
...
def compute_outcome(row):
    ...
    count_query ="SELECT weight, on_day FROM weigh_in_history WHERE ck_id = '"+ck_id+"' AND on_day >='"+join_date+"' ORDER BY on_day ASC" 
    
    ##print "query result", count_query
    weights = []
    ...
    for r in result:
       ...
       #print "list of weights", type(r['weight']) 
    ...
    if len(weights)>1:
        ...
        outcome = float(end_weight)-float(start_weight) 
...
def compute_outcome_percent(row):
    db = DBConnection()
    ck_id = row['ck_id']
    #print "ckid", ck_id
    join_date = str(row['join_date'].date())
    ...
    if len(weights) >1:
        ...
        end_weight = weights[1]
        try:
            outcome = 100*(float(end_weight) - float(start_weight))/float(start_weight) 
        except ZeroDivisionError:
            outcome = "NA"
    else:
        outcome = "NA"

    print "outcome_per", outcome
...
def compute_weighins_20(row):
    db = DBConnection() 
    ck_id = row['ck_id']
    join_date = str(row['join_date'].date())
    cut_off = str(datetime(*strptime(join_date, "%Y-%m-%d")[0:6]) + relativedelta(days = 20)) 
    count_query ="SELECT COUNT(*) FROM weigh_in_history WHERE ck_id = '"+ck_id+"' AND on_day >= '"+join_date+"' AND on_day < '"+cut_off+"' "
    count = int(db.query(count_query)[0]['COUNT(*)'])
    return count
...
class Everyone(object):
        """this class excludes new comers"""
        def __init__(self):
            self.connection = DBConnection()
            self.header = ["id","ck_id","join_date","initial_weight", "most_recent_weight", "height", "age",\
             "weighins", "initial_bmi", "final_bmi", "percentage_weight_change", "weight_change",\
             "time_in_system", "days", "outcome", "outcome_percent", "activity", "act20", "wi20"]
            self.header_types = ["s","s","s","n", "n", "n", "n",\
             "n","n", "n", "n", "n", "n", "n", "n", "n", "n", "n", "n"]
            self.table = []

        def build(self):
            build_query = """SELECT id,ck_id, join_date, initial_weight, most_recent_weight, height,
            age FROM users WHERE join_date > '2009-1-1'"""
...
class Adherent(Everyone):
    ...
    def write(self, adherent_filename, nonadherent_filename):
        ...
        nx.write_edgelist(self.nonadherent_graph,self.nonadherent_filename)
...
if __name__ == "__main__":
    ...
    al = Everyone()
    al.build()
    al.write("./method3_50/master_everyone")
    #n = Newcomers()
    #n.build()
    ...
    #a = Adherent()
    #a.build()
    #a.write("./method3_50/interim/master_adherent", "./method3_50/interim/master_nonadherent")
