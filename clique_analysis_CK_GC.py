...
It reads all the graph files at once (months, quarters, year) from the 2, 5 and 10-point folders and calculates the number of n-cliques just for the giant component of each network, being n=1,2,...,7

It prints out the results in a general 'summary_clique_analysis_GC' file.
...
file1 = open('summary_clique_analysis_GC','wt') 
print >> file1, "data:  #_points   time_scale   GC_size   n-clique:#_n-cliques   max_clique_size   num_tot_cliques"
...
for name in network_list: # loop to go over files (== over networks)
    ...
    G=components[0] # i take just the GC to perform the community ID algorithm
...
# count number of 2, 3, 4, 5, 6  and 7cliques:
    ...
    file1 = open('summary_clique_analysis_GC','at') 
